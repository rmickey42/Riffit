...
router.route("/:userId/likes").get(async (req, res) => {
  try {
    let id = req.params.userId;
    id = validation.checkId(id, "User Id");
    const user = await userData.getUserById(id);
...
router.route("/:userId/dislikes").get(async (req, res) => {
  try {
    let id = req.params.userId;
    id = validation.checkId(id, "User Id");
    const user = await userData.getUserById(id);
...
router.route("/:userId/favorites").get(async (req, res) => {
  try {
    let id = req.params.userId;
    id = validation.checkId(id, "User Id");
    const user = await userData.getUserById(id);
...
router
  ...
  .post(upload.any(), async (req, res) => {
    try {
      ...
      let userInfo = { bio, instruments, genres, currentPassword, newPassword, confirmPassword };
      ...
      if (currentPassword || newPassword || confirmPassword) {
        ...
        userInfo.password = validation.checkConfirmedPassword(
          newPassword,
          confirmPassword
        );
        const currentUser = await userData.getUserById(id, true);
